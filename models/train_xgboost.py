"""Train and evaluate an XGBoost model on the t+1 credit application dataset.
...
# were memorizing the training months (large train/validation PR-AUC gap).
HYPERPARAMETER_SEARCH_SPACE = {
    ...
    "reg_lambda": [1, 5, 10, 25, 50],
}

N_TRIALS = 50

# Boosting rounds; early stopping is kept short so training halts before it starts
# fitting noise that no longer improves validation PR-AUC.
N_ESTIMATORS = 500
...
def load_split(split_name: str) -> pd.DataFrame:
    """Load one split (train/val/test) of the xgboost_t1 dataset."""
...
def calculate_scale_pos_weight(train_df: pd.DataFrame) -> float:
    """Calculate the negative-to-positive class ratio using training data only."""
    n_negative = (train_df[TARGET_COLUMN] == 0).sum()
    n_positive = (train_df[TARGET_COLUMN] == 1).sum()

    if n_positive == 0:
        raise ValueError("Training data contains no positive observations.")
...
def train_xgboost_model(
    ...
    """Fit an XGBoost classifier using validation data for early stopping."""
...
def tune_hyperparameters(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        model = train_xgboost_model(train_df, val_df, feature_columns, params)
    ...
    # refit with the winning configuration so the returned model matches best_params
    best_model = train_xgboost_model(train_df, val_df, feature_columns, best_params)
...
def train_model(n_trials: int = N_TRIALS) -> tuple[xgb.XGBClassifier, list[str], pd.DataFrame]:
    ...
    print(f"  scale_pos_weight: {calculate_scale_pos_weight(train_df):.2f}")
